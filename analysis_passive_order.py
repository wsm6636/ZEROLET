...
def our_chain_order(tasks):
    """
    Our chain analysis with optimal initial merge position.
    All logic is merged into a single function.

    return:
        (max_reaction_time, final_r, final_w, merge_pair)
        False if no valid merge order exists
    """
    n = len(tasks)
    if n < 1:
        return False
    ...
    if n == 1:
        ...
        return max_reaction_time, final_r, final_w, None
    ...
    for k in range(n - 1):
        ...
        result = combine_no_free_jitter(tasks[k], tasks[k + 1])
        if result is False:
            continue
        ...
        for i in range(n):
            if i == k:
                if current_task is None:
                    ...
                    result = combine_no_free_jitter(current_task, merged_task)
                    if result is False:
                        valid = False
                        break
                    r, w = result
    ...
    return max_reaction_time, r1n, w1n, best_merge_pair
...
# outport function
def run_analysis_passive_our_order(num_tasks, periods,read_offsets,write_offsets, per_jitter):
    global results_function
    results_function = []  

    tasks = RandomEvent(num_tasks, periods,read_offsets,write_offsets, per_jitter).tasks

    final = our_chain_order(tasks)
    ...
    return final_e2e_max, max_reaction_time, final_r, final_w, tasks, best_merge_pair
...
if __name__ == "__main__":
    ...
    print("Best merge pair:", best_merge_pair)
