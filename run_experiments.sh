...
case "$TYPE" in
    ...
    PAORDER) ALG="PAORDER"; SUFFIX="_PAORDER" ;;
    ADORDER) ALG="ADORDER"; SUFFIX="_ADORDER" ;;
...
if [[ "$ALG" == "ADD" ]]; then    COMMON_CSV_ACTIVE="${OUT_DIR}/common_results_active${SUFFIX}_${TIMESTAMP}.csv"    COMMON_CSV_ACTIVE_ADD="${OUT_DIR}/common_results_active_add${SUFFIX}_${TIMESTAMP}.csv"

elif [[ "$ALG" == "PAORDER" ]]; then
COMMON_CSV_PASSIVE="${OUT_DIR}/common_results_passive${SUFFIX}_${TIMESTAMP}.csv"
COMMON_CSV_PASSIVE_ORDER="${OUT_DIR}/common_results_passive_order${SUFFIX}_${TIMESTAMP}.csv" 

else
COMMON_CSV_PASSIVE="${OUT_DIR}/common_results_passive${SUFFIX}_${TIMESTAMP}.csv"    COMMON_CSV_ACTIVE="${OUT_DIR}/common_results_active${SUFFIX}_${TIMESTAMP}.csv"
fi
...
for i in $(seq 1 $NUM_EXPERIMENTS); do
    ...
    if [[ "$ALG" == "ADD" ]]; then
        ...
        tmp_csv_active_add="${OUT_DIR}/tmp_${i}_active_add${SUFFIX}.csv"
        ...
        $PYTHON main.py "$seed" "$NUM_REPEATS" \
            ...
            --common_csv_active_add "$tmp_csv_active_add" \
            --suffix "$SUFFIX" \
            --alg "$ALG" \
            --periods "${PERIODS[@]}" \
            --jitters "${JITTERS[@]}" \
        ...
        pids+=($!)
            ...
            tmp_csv_passive_order="${OUT_DIR}/tmp_${i}_passive_order${SUFFIX}.csv"
            echo "Starting experiment $i/$NUM_EXPERIMENTS (PASSIVE NEW), seed=$seed"
            $PYTHON main.py "$seed" "$NUM_REPEATS" \
                --common_csv_passive "$tmp_csv_passive" \
                --common_csv_passive_order "$tmp_csv_passive_order" \
                --suffix "$SUFFIX" \
                --alg "$ALG" \
                --periods "${PERIODS[@]}" \
                --jitters "${JITTERS[@]}" \
...
if [[ "$ALG" == "ADD" ]]; then
    # Merge active and active_add
    head -n 1 "${OUT_DIR}/tmp_1_active${SUFFIX}.csv" > "$COMMON_CSV_ACTIVE"
    head -n 1 "${OUT_DIR}/tmp_1_active_add${SUFFIX}.csv"   > "$COMMON_CSV_ACTIVE_ADD"
    for i in $(seq 1 $NUM_EXPERIMENTS); do
        tail -n +2 "${OUT_DIR}/tmp_${i}_active${SUFFIX}.csv" >> "$COMMON_CSV_ACTIVE"
        tail -n +2 "${OUT_DIR}/tmp_${i}_active_add${SUFFIX}.csv"   >> "$COMMON_CSV_ACTIVE_ADD"
    done
    rm -f "${OUT_DIR}"/tmp_*"${SUFFIX}".csv
elif [[ "$ALG" == "PAORDER" ]]; then
    # Merge passive and passive_order
    head -n 1 "${OUT_DIR}/tmp_1_passive${SUFFIX}.csv" > "$COMMON_CSV_PASSIVE"
    head -n 1 "${OUT_DIR}/tmp_1_passive_order${SUFFIX}.csv"   > "$COMMON_CSV_PASSIVE_ORDER"
    for i in $(seq 1 $NUM_EXPERIMENTS); do
        tail -n +2 "${OUT_DIR}/tmp_${i}_passive${SUFFIX}.csv" >> "$COMMON_CSV_PASSIVE"
        tail -n +2 "${OUT_DIR}/tmp_${i}_passive_order${SUFFIX}.csv"   >> "$COMMON_CSV_PASSIVE_ORDER"
    done
    rm -f "${OUT_DIR}"/tmp_*"${SUFFIX}".csv
else
    head -n 1 "${OUT_DIR}/tmp_1_passive${SUFFIX}.csv"     > "$COMMON_CSV_PASSIVE"
...
if [[ "$ALG" == "ADD"  ]]; then
    if $PYTHON generate_comparison.py \
            ...
            --common_csv_active_add "$COMMON_CSV_ACTIVE_ADD" \
            --suffix "$SUFFIX"; then
    ...
    if $PYTHON generate_comparison.py \
            ...
            --common_csv_passive_order "$COMMON_CSV_PASSIVE_ORDER" \
            --suffix "$SUFFIX"; then
        echo "Final comparison plot (${TYPE}) generated successfully"
    else
        echo "Error occurred while generating comparison plot!"
