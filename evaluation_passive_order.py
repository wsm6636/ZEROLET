...
def output_passive_our_order(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters):
    ...
    with open(results_csv, mode='w', newline='') as file:
        ...
        for num_tasks in num_chains:
            for per_jitter in jitters:
                ...
                for (final_e2e_max, max_reaction_time, r, tasks, seed, exceed, best_merge_pair) in results[num_tasks][per_jitter]:
                    writer.writerow([seed,num_tasks, per_jitter, final_e2e_max, max_reaction_time, r, exceed, false_percentage, best_merge_pair])
...
def run_evaluation_passive_our_order(jitters, num_chains, num_repeats, random_seed, periods):
    TOLERANCE = 1e-9
...
if __name__ == "__main__":
    ...
    # results, false_results, final = run_evaluation_passive_our_order(jitters, num_chains, num_repeats, random_seed, periods)
