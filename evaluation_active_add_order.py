...
def output_active_our_add_order(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters):
    """
    ...
    """
    folder_path = "order/add"
    os.makedirs(folder_path, exist_ok=True)

    results_csv = os.path.join(folder_path, f"data_active_add_order_{num_repeats}_{random_seed}_{timestamp}.csv" )

    # save results to csv
    with open(results_csv, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["seeds","num_tasks", "per_jitter", "final_e2e_max", "max_reaction_time", "R", "exceed", "false_percentage", "best_merge_pair", "added"])
        for num_tasks in num_chains:
            for per_jitter in jitters:
                false_percentage = false_results[num_tasks][per_jitter]
                for (final_e2e_max, max_reaction_time, r, tasks, seed, exceed,best_merge_pair,added) in results[num_tasks][per_jitter]:
                    writer.writerow([seed,num_tasks, per_jitter, final_e2e_max, max_reaction_time, r, exceed, false_percentage, best_merge_pair, added])
...
"""For testing"""
def run_evaluation_active_our_add_order(jitters, num_chains, num_repeats, random_seed, periods):
    TOLERANCE = 1e-9
    ...
    for i in range(num_repeats):            # loop on number of repetitions
        ...
        for num_tasks in num_chains:        # on number of tasks in a chain
            ...
            for per_jitter in jitters:      # on relative (to period) magnitude of jitter
                ...
                final_e2e_max, max_reaction_time,  final_r, final_w, tasks, best_merge_pair, added = run_analysis_active_our_add_order(num_tasks, selected_periods,selected_read_offsets,selected_write_offsets, per_jitter)
                ...
                results[num_tasks][per_jitter].append((final_e2e_max, max_reaction_time,r,tasks,random_seed,exceed,best_merge_pair,added))
...
if __name__ == "__main__":
    ...
    results, false_results, final = run_evaluation_active_our_add_order(jitters, num_chains, num_repeats, random_seed, periods)
    output_active_our_add_order(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters)
